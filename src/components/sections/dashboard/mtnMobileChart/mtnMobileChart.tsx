...
type MtnMobileChartProps = {
  chartRef: React.MutableRefObject<EChartsReactCore | null>;
  data: MtnMobileStatisticsData[];
  sx?: SxProps;
};

type MtnMobileChartOptions = echarts.ComposeOption<
  BarSeriesOption | LegendComponentOption | TooltipComponentOption | GridComponentOption
>;

const MtnMobileTelcoChart = ({ chartRef, data, ...rest }: MtnMobileChartProps): ReactElement => {
  const theme = useTheme();

  const option: MtnMobileChartOptions = useMemo(
    () => ({
      tooltip: {
        trigger: 'axis',
        axisPointer: {
...
export default MtnMobileTelcoChart;
