...
type NineMobileChartProps = {
  chartRef: React.MutableRefObject<EChartsReactCore | null>;
  data: NineMobileStatisticsData[];
  sx?: SxProps;
};

type NineMobileChartOptions = echarts.ComposeOption<
  BarSeriesOption | LegendComponentOption | TooltipComponentOption | GridComponentOption
>;

const NineMobileTelcoChart = ({ chartRef, data, ...rest }: NineMobileChartProps): ReactElement => {
  const theme = useTheme();

  const option: NineMobileChartOptions = useMemo(
    () => ({
      tooltip: {
        trigger: 'axis',
        axisPointer: {
...
export default NineMobileTelcoChart;
