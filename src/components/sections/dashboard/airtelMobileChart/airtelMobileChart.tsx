...
type AirtelChartProps = {
  chartRef: React.MutableRefObject<EChartsReactCore | null>;
  data: AirtelStatisticsData[];
  sx?: SxProps;
};

type AirtelChartOptions = echarts.ComposeOption<
  BarSeriesOption | LegendComponentOption | TooltipComponentOption | GridComponentOption
>;

const AirtelMobileTelcoChart = ({ chartRef, data, ...rest }: AirtelChartProps): ReactElement => {
  const theme = useTheme();

  const option: AirtelChartOptions = useMemo(
    () => ({
      tooltip: {
        trigger: 'axis',
        axisPointer: {
...
export default AirtelMobileTelcoChart;
